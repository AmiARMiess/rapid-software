...
<head>
  <meta charset="utf-8">
  <title>Position {{ $position->name }} — Report</title>
  <style>
...
<body>
  <header>
    <div class="brand">
      <div>
        <h2>Company Name</h2>
        <p class="muted">Position Report</p>
      </div>
      <div style="margin-left:auto; text-align:right;">
        <p class="muted">Generated: {{ now()->format('d M Y H:i') }}</p>
        <p class="muted">Position ID: {{ $position->id }}</p>
      </div>
    </div>
  </header>
  ...
  <main>
      ...
      <div class="left">
        <div class="card">
          <div style="display:flex; align-items:center; width:100%; gap:16px;">
            <div style="flex:1;">
              <h3>Position</h3>
              <div style="font-size:16px; font-weight:700; color:#0b3a66;">{{ $position->name }}</div>
              <div class="muted">{{ optional($position->optionStatus)->name ?? 'Inactive' }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="section">
          ...
          <div class="card">
            <dl class="details">
              <dt>Created On</dt>
              <dd>{{ $position->created_at->format('d M Y') }}</dd>

              <dt>Description</dt>
              <dd class="description">{{ $position->description ?: 'No description provided.' }}</dd>
            </dl>
          </div>
        </div>
      </div>
      ...
      <div class="right">
        <div class="card" style="margin-bottom:12px;">
          ...
          <div style="display:flex; flex-direction:column; gap:8px;">
            ...
            <div style="display:flex; justify-content:space-between;"><div class="muted">Status</div><div>{{ optional($position->optionStatus)->name ?? 'Inactive' }}</div></div>
          </div>
        </div>
    ...
    <div class="section">
      ...
      <div class="card">
        @if($position->positionResponsibles->isEmpty())
        ...
        @else
          <table class="responsibilities">
            ...
            <tbody>
              @foreach ($position->positionResponsibles as $i => $responsibility)
                <tr>
